...
function checkDomainConsistency($object_id, &$conflicts = array(), $desired_domain_id = NULL) {
    $vswitch = getVLANSwitchInfo ($object_id);
    $object_8021q_domain_id = $vswitch['domain_id'];

    if (NULL == $desired_domain_id)
    {
        if(NULL == $object_8021q_domain_id)  { return array(); }                      /*no desired domain_id*/

        $desired_domain_id = $object_8021q_domain_id;
    }

    if ($object_8021q_domain_id != $desired_domain_id)                                /*compare domain_id's*/
    {
        $conflicts[$object_id] = array (
            'old_domain_id' => $object_8021q_domain_id,
            'new_domain_id' => $desired_domain_id
        );
    }
    if (NULL == $object_8021q_domain_id) { return $conflicts; }                         /* no need to search downlinks*/

    $ports = getObjectPortsAndLinks($object_id);
    $desired_config = apply8021QOrder ($vswitch, getStored8021QConfig ($object_id));
    foreach ($ports as $port) {                                                         /*process downlinks*/
        if
        (
            $port['linked'] &&
            isset($desired_config[$port['name']]) &&
            $desired_config[$port['name']]['vst_role'] == 'downlink'
        ){
            $downlinks[$port['name']] = $desired_config[$port['name']];
            checkDomainConsistency($port['remote_object_id'], $conflicts, $desired_domain_id);
        }
    }
